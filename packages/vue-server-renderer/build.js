...
TemplateRenderer.prototype.renderPreloadLinks = function renderPreloadLinks (context) {
  ...
  if (this.preloadFiles || usedAsyncFiles) {
    return (this.preloadFiles || []).concat(usedAsyncFiles || []).map(function (file) {
      ...
      var ext = path.extname(withoutQuery).slice(1);
      var type = getPreloadType(ext);
      ...
      return ("<link rel=\"preload\" href=\"" + (this$1.publicPath) + "/" + file + "\"" + (type !== '' ? (" as=\"" + type + "\"") : '') + extra + ">")
    }).join('')
  } else {
    return ''
  }
...
TemplateRenderer.prototype.renderState = function renderState (context) {
